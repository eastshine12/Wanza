...
ADD CONSTRAINT FK_QCOMMENT_USERSEQ FOREIGN KEY(USERSEQ)
...
CREATE TABLE CARD_REVIEW(
    ...
    REFERENCES CARD_COLLECTIONS(CARDSEQ),
    CONSTRAINT FK_REVIEW_USERSEQ FOREIGN KEY(USERSEQ)
    REFERENCES USERS(USERSEQ)
);
...
CREATE TABLE PRODUCTION_OPTION(
    PRODUCTOPTIONSEQ NUMBER(8) NOT NULL,
    PRODUCTSEQ NUMBER(8) NOT NULL,
...
SELECT PRODUCTQNASEQ, Q.PRODUCTSEQ, Q.USERSEQ, PURCHASESEQ, PRODUCTQCONTENT, PRODUCTACONTENT, PRODUCTQNADATE, PRODUCTQNADEL, NICKNAME, P.PRODUCTNAME
FROM (SELECT ROW_NUMBER()OVER(ORDER BY PRODUCTQNASEQ DESC) AS RNUM, PRODUCTQNASEQ, PRODUCTSEQ, USERSEQ, PURCHASESEQ, PRODUCTQCONTENT, PRODUCTACONTENT, PRODUCTQNADATE, PRODUCTQNADEL 
...
WHERE Q.USERSEQ=U.USERSEQ AND Q.PRODUCTSEQ=P.PRODUCTSEQ AND PRODUCTQNADEL=0 
    --AND PRODUCTACONTENT IS NOT NULL
...
SELECT C.CARDSEQ, C.CARDTITLE, C.CARDCONTENT, C.USERSEQ, C.CARDREADCOUNT, C.CARDDATE, C.CARDFILENAME,
        C.CARDDEL, U.NICKNAME, CF.LIKECOUNT, TAGCOUNT, TAGEXIST
FROM 
(SELECT ROW_NUMBER()OVER(ORDER BY CARDSEQ DESC) AS RNUM, CARDSEQ, CARDTITLE, CARDCONTENT, USERSEQ, CARDREADCOUNT, CARDDATE, CARDFILENAME, CARDDEL FROM CARD_COLLECTIONS
        WHERE CARDDEL=0
        ) C,
(SELECT * FROM USERS
) U,
(SELECT CARDSEQ, NVL(COUNT(*), 0) LIKECOUNT FROM CARD_FAVORITE GROUP BY CARDSEQ) CF,
(SELECT ROW_NUMBER()OVER(ORDER BY CC.CARDSEQ DESC) AS RNUM2, CC.CARDSEQ, COUNT(CC.PRODUCTSEQ) TAGCOUNT, NVL2(COUNT(CC.PRODUCTSEQ), 'TRUE', 'FALSE') TAGEXIST  
    FROM CARD_COORDINATES CC, CARD_COLLECTIONS C  WHERE C.CARDSEQ=CC.CARDSEQ(+) GROUP BY CC.CARDSEQ HAVING COUNT(CC.PRODUCTSEQ) > 0) T	
WHERE C.USERSEQ=U.USERSEQ AND C.CARDSEQ=CF.CARDSEQ(+) AND C.CARDSEQ=T.CARDSEQ(+)
AND RNUM2>=1 AND RNUM2<=10
ORDER BY C.CARDDATE DESC;


SELECT ROW_NUMBER()OVER(ORDER BY USERDATE ASC) AS RNUM, U.USERSEQ, AUTH, PROFILENAME, NICKNAME, USERNAME, EMAIL, PHONE, USERDATE, MILEAGE, ADDRESS
FROM USERS U, ADDRESS A
WHERE U.USERSEQ=A.USERSEQ(+) AND U.DEFAULTADDRESS=A.ADDRESSSEQ(+);

SELECT U.USERSEQ, AUTH, PROFILENAME, NICKNAME, USERNAME, EMAIL, PHONE, USERDATE, MILEAGE, ADDRESS
	FROM  
	(SELECT ROW_NUMBER()OVER(ORDER BY USERDATE ASC) AS RNUM, USERSEQ, AUTH, PROFILENAME, NICKNAME, USERNAME, EMAIL, PHONE, USERDATE, MILEAGE, DEFAULTADDRESS 
		FROM USERS) U, ADDRESS A
	WHERE U.USERSEQ=A.USERSEQ(+) AND U.DEFAULTADDRESS=A.ADDRESSSEQ(+)
	AND RNUM>=1 AND RNUM<=10;
...
SELECT PRODUCTREVSEQ, R.USERSEQ, PRODUCTREVCONTENT, PRODUCTREVFILENAME, PRODUCTREVDATE, PRODUCTREVRATING, NICKNAME
	FROM (SELECT ROW_NUMBER()OVER(ORDER BY PRODUCTREVDATE DESC) AS RNUM, PRODUCTREVSEQ, USERSEQ, PRODUCTREVCONTENT, PRODUCTREVFILENAME, PRODUCTREVDATE, PRODUCTREVRATING FROM PRODUCTION_REVIEW WHERE PRODUCTSEQ=45 AND PRODUCTREVDEL=0) R, USERS U
	WHERE R.USERSEQ=U.USERSEQ
	
    AND PRODUCTREVRATING=4
	ORDER BY PRODUCTREVDATE DESC;
...
WHERE O.ORDERSEQ=C.ORDERSEQ AND C.PRODUCTSEQ=P.PRODUCTSEQ AND O.ORDERSEQ=119;

SELECT ADDRESSNAME, ADDRESS, RECEIVEUSER, RECEIVEPHONE, ORDERMESSAGE
FROM ORDERS O, ADDRESS A
WHERE O.ADDRESSSEQ=A.ADDRESSSEQ AND O.ORDERSEQ=125;
